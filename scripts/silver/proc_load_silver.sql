...
BEGIN
	...
	BEGIN TRY
		...
		FROM bronze.crm_cust_info ) t  -- use row_number to fetch the record of latest date
		WHERE flag_last = 1;
		...
		-- SELECT
		SELECT prd_id, 
		REPLACE(SUBSTRING(prd_key,1,5),'-','_') as cat_id, --erp_px_cat_g1v2
		SUBSTRING(prd_key,7,LEN(prd_key)) as prd_key, -- crm_sales_details
		prd_nm, 
